...
public class LoomClientRequestImpl<T extends RestResponseModel> implements LoomClientRequest<T> {
    ...
    @Override
    public LoomClientRequest<T> addWait(boolean wait) {
        return addQueryParameter("wait", String.valueOf(wait));
    }

    @Override
    public LoomClientRequest<T> addTimeout(int timeout) {
        return addQueryParameter("timeout", String.valueOf(timeout));
    }

    @Override
    public LoomClientRequest<T> addForce(boolean force) {
